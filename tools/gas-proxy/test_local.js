...
{ const d = store['data/weekly/sessions/2026-09-20.json']; d.date = '2099-01-01';
  ...
  g = call({ ...base, op: 'edit', base: g.rev, prev: { s0c1: [['p:zz', 'p:zz'], ['p:c', 'p:d']] }, value: [{ id: 's0c1', aIds: ['p:a', 'p:b'], bIds: ['p:c', 'p:d'] }] }); results.push(['edit prev mismatch → STALE', g.code === 'STALE']);
  { const cur = store['data/weekly/sessions/2026-09-20.json'].schedule.matches.find((x) => x.id === 's0c1'); g = call({ ...base, op: 'edit', base: g.rev, prev: { s0c1: [cur.aIds.slice(), cur.bIds.slice()] }, value: [{ id: 's0c1', aIds: ['p:a', 'p:b'], bIds: ['p:c', 'p:d'] }] }); results.push(['edit prev match ok', g.ok]); }
  g = call({ ...base, op: 'edit', base: g.rev, prev: 'bad', value: [{ id: 's0c1', aIds: ['p:a', 'p:b'], bIds: ['p:c', 'p:d'] }] }); results.push(['edit prev malformed → INVALID', g.code === 'INVALID']);
...
{ const app = fs.readFileSync(__dirname + '/../../app.js', 'utf8');
  ...
  const seq = [
    ...
    { op: 'edit', base: 7, prev: { s0c1: [['p:a', 'p:b'], ['p:c', 'p:d']] }, value: [{ id: 's0c1', aIds: ['p:a', 'p:x'], bIds: ['p:c', 'p:d'] }] }, { op: 'edit', base: 8, prev: { s0c1: [['p:a', 'p:b'], ['p:c', 'p:d']] }, value: [{ id: 's0c1', aIds: ['p:a', 'p:y'], bIds: ['p:c', 'p:d'] }] }, { op: 'edit', base: 8, value: [{ id: 's0c1', aIds: ['p:a', 'p:x'], bIds: ['p:c', 'p:x'] }] }, { op: 'edit', base: 99, value: [{ id: 's0c1', aIds: ['p:a', 'p:b'], bIds: ['p:c', 'p:d'] }] },
